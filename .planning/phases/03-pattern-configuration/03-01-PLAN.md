...
**Scope Note:** FR-3 specifies "Support multiple pattern files (per-project)". Per research (03-RESEARCH.md), single pattern file is recommended for v1 to keep complexity manageable. Multiple pattern file support is DEFERRED to a future phase. This plan implements single-file pattern loading only.
</objective>
...
3. Add field_validator on EntityPattern.pattern that:
   ...
   - Rejects patterns with 0 capturing groups: raises ValueError("Pattern must have exactly one capture group, got 0")
   - Rejects patterns with 2+ capturing groups: raises ValueError(f"Pattern must have exactly one capture group, got {n}")
   - Raises ValueError with clear message for invalid regex syntax
...
python -c "from cosmograph.config.patterns import EntityPattern; EntityPattern(name='no_group', pattern='[a-z]+', category='test')" 2>&1 | grep -q "capture group"
python -c "from cosmograph.config.patterns import EntityPattern; EntityPattern(name='multi', pattern='([a-z]+)([0-9]+)', category='test')" 2>&1 | grep -q "capture group"
  </verify>
  <done>
PatternConfig models import cleanly, valid patterns pass validation, invalid regex rejected with clear error, 0 capture groups rejected, 2+ capture groups rejected.
  </done>
</task>
...
- Capture group validation rejects 0 groups and 2+ groups with specific messages
- PyYAML added to core dependencies
