...
# Create test pattern file and test document
python -c "
import tempfile
import yaml
...
with tempfile.NamedTemporaryFile(mode='w', suffix='.txt', delete=False) as doc:
    doc.write('Contact test@example.com for info. Email test@example.com again.')
    doc_path = doc.name
...
# Run CLI with custom patterns (substitute paths from above)
# cosmograph generate /path/to/test.txt -e generic --patterns /path/to/patterns.yaml

# Test error handling with invalid pattern file
python -c "
import tempfile
with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
...
"
# cosmograph generate /path/to/test.txt -e generic --patterns /path/to/invalid.yaml
# Should show: "Error: Invalid patterns file: ..."
  </verify>
  <done>
...
2. Test extraction with custom patterns file:
   ...
   - Create test.txt with email addresses
   - Run `cosmograph generate ./test.txt -e generic --patterns test.yaml`
   - Verify emails extracted
3. Test error handling:
   - Invalid YAML syntax: `cosmograph generate ./test.txt -e generic --patterns invalid.yaml` shows "Error: Invalid patterns file: ..."
   - Invalid regex in patterns: clear error message
   - File not found: clear error message (handled by typer exists=True)
...
- Invalid pattern files show helpful error messages (tested with invalid YAML)
