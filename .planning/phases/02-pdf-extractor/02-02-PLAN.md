...
must_haves:
  truths:
    - "User can run 'cosmograph generate path/ -e pdf' and process PDF files"
    - "Valid PDFs with legal content produce graphs with document and entity nodes"
    - "Encrypted PDFs produce clear error message mentioning 'password-protected'"
    - "Scanned/image-only PDFs produce clear error message mentioning 'scanned'"
    - "3-page PDF produces entities from all pages in correct order"
  artifacts:
...
4. Add fixture `create_multipage_ordered_pdf` for structure preservation testing:
   ```python
   @pytest.fixture
...
grep -c "create_test_pdf\|create_encrypted_pdf\|create_image_only_pdf\|create_multipage_ordered_pdf" tests/conftest.py
```
  </verify>
  <done>PDF fixture generators exist in conftest.py including multi-page ordered fixture</done>
</task>
...
2. Add test class:
   ...
   class TestPdfExtractor:
       ...
       def test_multipage_preserves_reading_order(self, extractor, create_multipage_ordered_pdf):
           ...
           title_labels = [l for l in labels if "Title" in l]
           ...
           assert any("Title I" in l or "Title 1" in l for l in labels), "Missing Title I from page 1"
           assert any("Title II" in l or "Title 2" in l for l in labels), "Missing Title II from page 2"
           assert any("Title III" in l or "Title 3" in l for l in labels), "Missing Title III from page 3"
...
```
  ...
  <done>All TestPdfExtractor tests pass including structure preservation test</done>
</task>
...
# Verify structure preservation test specifically
pytest tests/test_extractors.py::TestPdfExtractor::test_multipage_preserves_reading_order -v
...
- TestPdfExtractor class exists with at least 6 test methods (including structure preservation)
- All PDF tests pass
